...
<body class="bg-[#FDFDFC] dark:bg-[#0a0a0a] text-[#1b1b18] dark:text-[#EDEDEC] font-sans min-h-screen flex flex-col">
    <header class="p-6 flex items-center justify-between">
        ...
        <!-- Mostrar "Crear Nuevo Producto" solo para admin -->
        @if(session('admin') && session('admin')->rol === 'admin')
            ...
            <div class="swiper-wrapper">
                ...
                <div class="swiper-slide">
                    <div class="product-info">
                        ...
                        <!-- Botón para "Agregar al carrito" (visible para todos) -->
                        <button class="px-4 py-2 bg-[#1b1b18] dark:bg-[#EDEDEC] text-white dark:text-[#1b1b18] rounded">
                            Agregar al carrito
                        </button>

                        <!-- Opciones CRUD: solo para administradores -->
                        @if(session('admin') && session('admin')->rol === 'admin')
                        <a href="{{ route('productos.edit', ['producto' => $producto->idProducto]) }}"
                            class="inline-block mt-2 px-4 py-2 bg-yellow-500 text-white rounded">
                            Editar
                        </a>
                        <form action="{{ route('productos.destroy', ['producto' => $producto->idProducto]) }}"
                            method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="inline-block mt-2 px-4 py-2 bg-red-600 text-white rounded"
